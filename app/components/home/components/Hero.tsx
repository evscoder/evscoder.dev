      ...
      <div className="container relative z-10 flex min-h-[calc(100svh-112px)] flex-col p-5 sm:p-8 lg:p-10">
        <header className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className={cn("hidden items-center gap-2 rounded-full border px-4 py-2 text-[10px] uppercase tracking-[.2em] sm:flex", isThemeAlt ? "border-white/10 bg-white/[.04] text-white/60" : "border-slate-200 bg-white/70 text-slate-600")}>
              ...
              {language === "ru" ? "Открыт к проектам" : "Available for projects"}
            </div>
        ...
        <div className="grid flex-1 items-center gap-10 py-14 lg:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)] lg:py-10">
          <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75, ease: [0.16, 1, 0.3, 1] }} className="max-w-[800px]">
            <div className={cn("mb-7 flex items-center gap-3 text-[10px] font-semibold uppercase tracking-[.28em]", isThemeAlt ? "text-cyan-200/60" : "text-cyan-800/70")}><Sparkles className="h-4 w-4" />{language === "ru" ? "Сложный frontend без сложности для людей" : "Complex frontend made simple for people"}</div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.92, x: 30 }} animate={{ opacity: 1, scale: 1, x: 0 }} transition={{ delay: 0.16, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="relative mx-auto w-full max-w-[500px]">
            ...
            <div className={cn("relative overflow-hidden rounded-[32px] border p-3 shadow-2xl backdrop-blur-2xl", isThemeAlt ? "border-white/10 bg-white/[.055] shadow-black/40" : "border-white bg-white/65 shadow-slate-400/20")}>
              <div className={cn("rounded-[24px] border p-5 sm:p-6", isThemeAlt ? "border-white/10 bg-[#0a0e17]/85" : "border-slate-200/80 bg-white/80")}>
        ...
        <footer className={cn("grid gap-3 border-t pt-5 sm:grid-cols-3", isThemeAlt ? "border-white/10" : "border-slate-200")}>
          {metrics.map((metric, index) => <div key={metric.value} className={cn("flex items-baseline gap-3 sm:px-5", index > 0 && (isThemeAlt ? "sm:border-l sm:border-white/10" : "sm:border-l sm:border-slate-200"))}><span className="text-2xl font-black tracking-tight">{metric.value}</span><span className={cn("text-[9px] uppercase tracking-[.2em]", isThemeAlt ? "text-white/40" : "text-slate-500")}>{metric[language]}</span></div>)}
        </footer>
      </div>
    </section>
  );
