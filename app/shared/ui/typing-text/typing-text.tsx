...
export function TypingText({
  ...
  useEffect(() => {
    if (reduced || !wordsRef.current.length) {
      return;
    }
  ...
  }, [wordsKey, typingSpeed, deletingSpeed, pause, reduced]);
  ...
  return (
    <span className="relative inline-grid items-center overflow-hidden align-baseline">
      ...
      <span className="absolute inset-0 inline-block align-middle whitespace-pre">
        <span className="inline-block">{reduced ? words[0] : displayText}</span>
        <span
          ref={cursorRef}
          className={reduced ? 'hidden' : 'ml-1 inline-block align-middle h-[1em] w-px bg-current'}
        />
      </span>
    </span>
  );
